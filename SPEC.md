...
APIVR-Δ v3.7.0 integrates CRYSTALIUM as the **primary backing store**, with the
...
APIVR-Δ v3.7.0 targets **ECL v2.0** (see `ECL_VERSION` at the repo root).
...
APIVR-Δ v3.7.0 accepts ECL envelopes matching `^2\.0(\.\d+)?$`. Receivers on VIGIL and FORGE are not yet ECL-adopters; emit is one-way until those Eidolons adopt (see `DESIGN-RATIONALE.md` §Future work).
