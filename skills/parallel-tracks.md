...
> **host-interpreted methodology**, not a mechanical runtime. APIVR-Δ does not
...
**`apivr-completion-report`** envelope (`templates/apivr-completion-report.envelope.json`,
profile `schemas/apivr-completion-report-profile.v1.json`). **No new ECL kind**
...
4. **Emit the aggregation artifact:** `templates/tracks-merge-report.md` — a
   ...
   `apivr-completion-report` envelopes: per-track table (id, worktree, file
...
- Parallel WRITE requires `isolation: worktree` — APIVR-Δ never fans out into a
  shared tree (SPEC.md I-8).
- Max 5 tracks; merge is single-threaded under continuous parent context.
- Reuse the existing `apivr-completion-report` (per track) and
